...
field_schema:
  ...
  PROJECT_DETAIL:              # Feeds: info-card (left_column) · summary-block (right_column) · meta claims block (right_column — communication_claims)
    ...
    - performance_summary:     # AI-authored summary rendered as ONE right-column card. Source strictly from CONVICTION_PERFORMANCE / CONSUMER_PERCEPTION; see the By-Type Contract below.
...
The sole `summary-block` must be a compact synthesis inside ONE SVG: `OVERALL` always present, plus a bold-subtitled line for each report type that actually has data in the JSON (per the By-Type Contract below):

1. **Field order and content — mandatory subtitles**
   - The card is a vertical stack of labeled lines. Every line starts with an **uppercase bold neutral subtitle** rendered as its own `<tspan font-weight="bold">`:
