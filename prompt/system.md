...
| `summary-block`  | **exactly 1**                      | one right-column summary card: `OVERALL` (always) + a labeled line per report type **present in the JSON** (`CLINICAL` / `SENSORY` / `INSTRUMENTAL` / `CONSUMER`) + a labeled line per present ISO country code (`CN` / `US` / `FR` / ...). OMIT every by-type line whose type has no data; OMIT the entire by-region sub-section when **every** result's `study_region` is `null`. The three sub-sections (OVERALL / by-type / by-region) stack inside ONE `<svg>` — never emit a second `summary-block`. |
